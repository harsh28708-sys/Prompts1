...
response that isn't valid JSON is retried once with a stricter reminder; if it still
can't be parsed, it falls back to score=0 rather than raising.
"""
...
async def judge_call_result(
    ...
    """Score one LLMCallResult. Never raises: execution failures and unparseable
    judge responses both resolve to a JudgeResult with score=0."""
    ...
    return JudgeResult(score=0, reasoning="judge parse failure", call_id=call_id)
